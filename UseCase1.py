...
class Fields(Frame):
    ...
    def clone(self):
        ...
        widget = OptionMenu(window, var1, '','', 'ssn','name','country','date', 'company' )
        #dropDown1.grid(column=2, row=1, sticky=W) 



        #widget = Label(self, text = 'Label #%s'%self.number)
        
        widget.grid(column = 2, row = %self.number)
        ent1.grid(column = 0, row = %self.number)
        self.widgets.append(ent1)
